"""
Chronos-2 forecast visualizer — matplotlib dark-theme PNG.

Produces a 2-row chart:
  Row 1 — last N candlesticks + 5 coloured percentile lines (P05/P25/P50/P75/P95)
           + ground truth overlay (faded) + right-side value labels
  Row 2 — P(price > last close) at each future bar, green/red fill around the 50% line

Usage (script):
    python Chronos/chronos_plots.py
...
# Percentile lines: (quantile, color, linewidth, linestyle, label)
# Red = pessimistic low end → gold = median → teal = optimistic high end
_PLINES = [
    (0.05, "#ef5350", 1.0, "--", "P05"),
    (0.25, "#ff8a65", 1.2, "--", "P25"),
    (0.50, "#ffc107", 2.2, "-",  "P50"),
...
def _draw_candles(ax: plt.Axes, df: pd.DataFrame, alpha: float = 1.0) -> None:
    """Draw OHLCV candlesticks on `ax`. df must have a DatetimeIndex."""
    if df.empty:
        return

    xs = mdates.date2num(df.index.to_pydatetime())
    if len(xs) >= 2:
        bar_w = 0.55 * (xs[1] - xs[0])
    else:
        bar_w = 0.0004  # fallback for single bar
...
def plot_forecast(
    ...
    *,
    ...
    """
    Build a matplotlib forecast chart from a ChronosForecaster result dict.
    ...
    label_x = fc_x[-1] + fc_span * 0.01  # just past final bar for labels
    ...
    # Set x range: visible context + forecast + label margin
    margin_l = (fc_x[-1] - ctx_x[0]) * 0.01
    margin_r = fc_span * 0.18  # room for right-side labels
    for ax in (ax1, ax2):
    ...
    parts.append("Chronos-2 Zero-Shot")
    ...
    if gt_df is not None and len(gt_df) > 0:
        handles.append(
            mpatches.Patch(facecolor=_TEAL, alpha=0.38, edgecolor="none", label="Ground truth")
        )
...
def save_png(fig: plt.Figure, path: str = "chronos_forecast.png", dpi: int = 150) -> str:
    fig.savefig(path, dpi=dpi, bbox_inches="tight", facecolor=fig.get_facecolor())
    print(f"[Chronos] Saved → {path}")
    return path
...
if __name__ == "__main__":
    ...
    OUT_PNG      = _Path(__file__).parent.parent / "chronos_forecast.png"
    ...
    forecaster = ChronosForecaster(context_length=512)
