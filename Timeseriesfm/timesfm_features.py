...
timesfm_features.py
===================
TimesFM 2.5 quantile feature generator for XGBoost.

Slides a context window along historical data, running TimesFM every
...
For each combination of percentile p and horizon h:
    tfm_q{pp}_h{h}    normalised predicted price at horizon h bars ahead
                      (pp = int(p*100) zero-padded, e.g. tfm_q10, tfm_q50, tfm_q90)
                      pp=00 encodes the mean (arithmetic), 10..90 are true percentiles
Additional columns:
    run_id            which TimesFM run produced this row
    staleness         bars since the last TimesFM run (0 = fresh run bar,
                      1…calc_interval-1 = copied from previous run)

Example: horizons=[5,10], percentiles=[0.1,0.5,0.9]
    → columns tfm_q10_h5  tfm_q50_h5  tfm_q90_h5
              tfm_q10_h10 tfm_q50_h10 tfm_q90_h10
              run_id  staleness
...
--------------
    ...
    df = generate(pair="EURUSD", years=[2023], timeframe="H1",
                  horizons=[5, 10, 15], percentiles=[0.1, 0.5, 0.9])
"""
...
def _find_proj() -> Path:
    ...
    while p != p.parent:
        if (p / "Pipeline").is_dir() and (p / "Timeseriesfm").is_dir():
            return p
        p = p.parent
...
from Timeseriesfm.timesfm_inference import TimesFMForecaster, MODEL_QUANTILES  # noqa: E402


def _pname(p: float) -> str:
    """Convert quantile float to two-digit column prefix: 0.1 → 'tfm_q10'."""
    return f"tfm_q{int(round(p * 100)):02d}"


# ─────────────────────────────────────────────────────────────────────────────
# Normalization (identical logic to chronos_features._normalize)
# ─────────────────────────────────────────────────────────────────────────────
...
def _normalize(
    ...
    """
    Normalize a (n_quantiles, pred_len) matrix of raw TimesFM prices.

    raw            : (n_q, pred_len) — TimesFM prices, each row one quantile path
...
def generate(
    ...
    """
    Generate TimesFM quantile features and save to parquet.
    ...
    context_length   : bars of history fed to TimesFM per run (≤ 16 384)
    horizons         : prediction horizons to record, e.g. [5, 10, 15, 20].
                       TimesFM runs up to max(horizons) bars ahead.
                       Defaults to [5, 10, 15, 20].
    percentiles      : quantile levels to record, e.g. [0.1, 0.5, 0.9].
                       Must be in {0.0, 0.1, …, 0.9} (TimesFM fixed levels).
                       Use 0.0 for the arithmetic mean.
                       Defaults to [0.1, 0.3, 0.5, 0.7, 0.9].
    calc_interval    : re-run TimesFM every N bars; copy predictions to next
                       N-1 bars (staleness 1…N-1). Default 1 = run every bar.
    norm_method      : "log_returns" | "fracdiff" | "raw"
    ...
    device           : "auto" | "gpu" | "cpu"
    save_plots       : save a TimesFM fan-chart PNG per run into save_dir/plots/

    Returns
    -------
    pd.DataFrame
        DatetimeIndex aligned to the resampled timeframe bars.
        Columns: tfm_q{pp}_h{h} for each percentile × horizon combination,
    ...
    assert max_horizon <= context_length, \
        f"max_horizon ({max_horizon}) must be ≤ context_length ({context_length})"
    assert calc_interval >= 1, "calc_interval must be ≥ 1"
    assert all(h >= 1 for h in horizons), "All horizons must be ≥ 1"
    assert all(p in MODEL_QUANTILES for p in percentiles), \
        f"Percentiles must be in {MODEL_QUANTILES}. Got: {percentiles}"

    # Map percentile → column index in quantile_matrix (10, pred_len)
    q_indices = [MODEL_QUANTILES.index(p) for p in percentiles]
    ...
    print(f"[TimesFM-feats] {pair} {timeframe}: {len(df):,} bars  "
    ...
    if save_plots:
        plot_dir = save_dir / "plots" / f"{pair}_{timeframe}_tfm"
    ...
    for run_id, i in enumerate(run_positions):
        ...
        q_matrix = fc["quantile_matrix"]   # (10, max_horizon)

        # (n_selected_q, max_horizon) — requested quantile rows
        raw_matrix = np.stack([q_matrix[qi] for qi in q_indices], axis=0)

        # (n_selected_q, max_horizon) — normalised
        ...
        for k in range(steps):
            ...
            for h in horizons:
                for qi_local, pn in enumerate(pnames):
                    row[f"{pn}_h{h}"] = float(norm_matrix[qi_local, h - 1])
            rows.append(row)
