"""
timesfm_feature_plot.py
=======================
Compare TimesFM 2.5 quantile predictions from a featdata parquet against the
...
Quantile columns (tfm_q{pp}_h{hh}) are discovered dynamically from the schema.
...
Usage (notebook / REPL):
    ...
    fig = plot_file("featdata/EURUSD_H1_tfm_ctx512_int10_h5-10-15-20_logret_wfilled_snone_2023.parquet",
                    plot_horizon=10)
    fig.show()
...
------
  ...
  Row 2 (30%) — forecast error: tfm_q50_h{plot_horizon} − actual
  Row 3 (15%) — staleness (bars since last TimesFM run, 0 = fresh)
"""
...
_RANK_STYLES: list[dict[str, Any]] = [
    dict(color="#ef5350", width=0.8, dash="dot"),
    dict(color="#ff8a65", width=1.0, dash="dash"),
    dict(color="#ffc107", width=2.0, dash="solid"),
    dict(color="#80cbc4", width=1.0, dash="dash"),
    dict(color="#26a69a", width=0.8, dash="dot"),
]
...
_COL_RE = re.compile(r"^tfm_q(\d+)_h(\d+)$")


def _discover_schema(df: pd.DataFrame) -> dict[int, list[str]]:
    """
    Scan df.columns for TimesFM quantile columns: tfm_q{pp}_h{hh}.
    Returns {horizon: [col, col, ...]} sorted by percentile within each horizon.
    """
...
_FNAME_RE = re.compile(
    r"^(?P<pair>[A-Z]+)_(?P<tf>[A-Z0-9]+)_tfm"
    r"_ctx(?P<ctx>\d+)"
    r"_int(?P<intv>\d+)"
    r"(?:_h(?P<horizons>[\d]+(?:-[\d]+)*))?"
...
def _parse_fname(fname: str) -> dict:
    ...
    if not m:
        raise ValueError(
            ...
            f"Expected format: PAIR_TF_tfm_ctxN_intN_h<H1>-<H2>-..._<norm>_YEAR.parquet"
        )
    g = m.groupdict()
...
def _build_figure(
    ...
    # prefer tfm_q50 as the error reference; fall back to middle column
    q50_col = next((c for c in q_cols if c.startswith("tfm_q50_")), q_cols[n_q // 2])
    ...
    title = (
        ...
        f"TimesFM 2.5  ctx={params['context_length']} "
    ...
    for col, style in zip(q_cols, styles):
        pct_label = col.split("_")[1].upper()   # e.g. "Q50"
        fig.add_trace(go.Scatter(
            x=x, y=df[col],
...
def plot_file(
    ...
    """
    Load a TimesFM features parquet, run the matching pipeline, and build the
    comparison figure.
    ...
    plot_horizon : which horizon to display (e.g. 5 shows tfm_q{pp}_h5).
                   Defaults to the smallest horizon present in the parquet.
    threshold    : fracdiff weight truncation threshold (must match generation params)
    ...
    tfm_df = pd.read_parquet(parquet_path)
    print(f"[Loaded] {parquet_path.name}  ({len(tfm_df):,} rows)")

    schema = _discover_schema(tfm_df)
    if not schema:
        raise ValueError(
            f"No TimesFM quantile columns (tfm_q{{pp}}_h{{hh}}) found in "
            f"{parquet_path.name}. Columns: {list(tfm_df.columns)}"
        )
    ...
    df = tfm_df.join(norm_close.rename("actual"), how="left")
...
if __name__ == "__main__":
    PARQUET      = "EURUSD_H1_tfm_ctx512_int5_h5-10-15-20_fdiff0.3_wfilled_snone_2023.parquet"
